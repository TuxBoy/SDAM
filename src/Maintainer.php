...
class Maintainer
{
		...
		foreach ($this->entities as $entity) {
			...
			foreach ($properties as $property) {
				...
				if (Annotation::of($entity)->hasAnnotation(AnnotationsName::C_FACTORY)) {
					$factoryValue = Annotation::of($entity)->getAnnotation(AnnotationsName::C_FACTORY)
						->getValue();
			...
			$migrationQueries = $currentSchema->getMigrateToSql(
				$schema, $this->connection->getDatabasePlatform()
			);
			$this->connection->transactional(
				function () use ($migrationQueries, $table) {
	...
	private function annotationIs($entity, string $propertyName, string $value): bool
	{
		return Annotation::of($entity, $propertyName)->hasAnnotation(AnnotationsName::P_LINK)
			&& Annotation::of($entity, $propertyName)->getAnnotation(AnnotationsName::P_LINK)->getValue()
			=== $value;
	}
	...
	{
		return !boolval(
			Annotation::of($entity, $propertyName)
	...
	private function addOptions(ReflectionProperty $property, string $entity, array $annotations): array
	{
		...
		foreach ($annotations as $name => $value) {
			...
			if (
				...
				$defaultMethod = Annotation::of($entity, $propertyName)->getAnnotation(
					AnnotationsName::P_DEFAULT
				)->getValue();
			...
			if (Annotation::of($entity, $propertyName)->hasAnnotation(AnnotationsName::P_NULL)) {
	...
	private function dropColumn(array $properties, Table $table)
	{
		...
		$columns         = array_filter(
			$columns, function (Column $column) {
			return !$this->tool->isForeignKey($column->getName());
		}
		);
		$arrayProperties = [];
		array_map(
			function (ReflectionProperty $property) use (&$arrayProperties) {
